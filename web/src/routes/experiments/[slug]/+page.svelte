<script lang="ts">
  ...
  const expDirectory: Record<string, { id: string; title: string; sub: string }> = {
    "unsafe-boundaries": {
      id: "02",
      title: "Unsafe Boundaries",
      sub: "Deliberately breaking memory safety to test FFI boundaries and raw pointers."
    },
    "async-runtime": {
      id: "03",
      title: "Async Runtime & Wakers",
      sub: "Building a custom executor to understand polling, pinning, and waker logic."
    },
    "cache-locality": {
      id: "04",
      title: "Cache Locality Benchmarks",
      sub: "Benchmarking Structure of Arrays (SoA) vs Array of Structures (AoS) layouts in real-time."
    }
  };

  $: info = expDirectory[slug] || {
    id: "0X",
    title: "Systems Experiment",
    sub: "This experiment is currently being structured in the core repository."
  };
  ...
  const memeImages = [
    ...
    },
    {
      ...
      caption: "Me deciding whether to write a safe abstraction or use an unsafe raw pointer."
    },
    {
      url: "https://i.imgflip.com/26am.jpg",
      title: "Ancient Aliens",
      caption: "Why did the FFI boundary execute with zero segmentation faults? ... RUST."
    },
    {
      url: "https://i.imgflip.com/1bij.jpg",
      title: "One Does Not Simply",
      caption: "One does not simply write bare-metal kernel hooks without testing invariants."
    },
    {
      url: "https://i.imgflip.com/4t0m5.jpg",
      title: "Disaster Girl",
      caption: "Me watching the legacy system crash while our new Rust experiment runs cleanly."
    }
  ];
  ...
  <header class="border-b border-white/[0.06] bg-[#090a0d]/90 backdrop-blur-md sticky top-0 z-40">
    <div class="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
      <div class="flex items-center gap-6">
        ...
        <span class="text-xs font-mono text-slate-400">EXP-{info.id} • {info.title}</span>
      </div>
  ...
  <div class="max-w-3xl mx-auto px-6 pt-16">
    ...
    <div class="fade-in-up opacity-0 translate-y-3 mb-12 text-center space-y-4">
      <div class="inline-block px-3 py-1 rounded-md bg-white/[0.04] border border-white/10 text-xs font-mono text-slate-400">
        EXP-{info.id} • UNDER CONSTRUCTION
      </div>
