<script lang="ts">
  ...
  const benchDirectory: Record<string, { label: string; sub: string }> = {
    "throughput": {
      label: "Throughput Benchmarks",
      sub: "Ops/sec, bandwidth saturation, and zero-copy packet processing rates."
    },
    "latency": {
      label: "Latency & Jitter Analysis",
      sub: "p99 / p99.9 latency distributions and kernel scheduling noise."
    },
    "memory": {
      label: "Memory Allocator Pressure",
      sub: "Tracking jemalloc vs system allocator performance and cache miss ratios."
    }
  };

  $: info = benchDirectory[slug] || {
    label: "Performance Benchmark",
    sub: "This benchmark suite is being calibrated in the test harness."
  };
  ...
  const memeImages = [
    ...
    },
    {
      ...
      caption: "Me deciding whether to optimize cache locality or sleep for 8 hours."
    },
    {
      url: "https://i.imgflip.com/26am.jpg",
      title: "Ancient Aliens",
      caption: "Why did the benchmark process 10 million ops/sec with zero latency jitter? ... RUST."
    },
    {
      url: "https://i.imgflip.com/1bij.jpg",
      title: "One Does Not Simply",
      caption: "One does not simply achieve linear scalability without lock-free data structures."
    },
    {
      url: "https://i.imgflip.com/4t0m5.jpg",
      title: "Disaster Girl",
      caption: "Me watching the Python benchmark melt the CPU while our Rust daemon stays at 1% load."
    }
  ];
...
<div class="min-h-screen bg-[#090a0d] text-slate-300 font-sans selection:bg-emerald-500/80 selection:text-white pb-32">
  
  <!-- Sticky Header -->
  <header class="border-b border-white/[0.06] bg-[#090a0d]/90 backdrop-blur-md sticky top-0 z-40">
    <div class="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
      <div class="flex items-center gap-6">
        <a href="/benches" class="inline-flex items-center gap-2 text-xs font-mono text-slate-400 hover:text-white transition-colors no-underline">
          <ArrowLeft size={14} />
          <span>Benches</span>
        </a>
        <div class="h-4 w-px bg-white/10"></div>
        <span class="text-xs font-mono text-slate-400">BENCH • {info.label}</span>
      </div>
      <div class="flex items-center gap-3 text-xs font-mono text-slate-400">
        <span class="px-2.5 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-medium">Coming Soon</span>
      </div>
    </div>
  </header>
    ...
    <div class="fade-in-up opacity-0 translate-y-3 p-6 md:p-8 rounded-2xl bg-[#0d0e12] border border-white/[0.08] space-y-6 flex flex-col items-center shadow-2xl">
      ...
      <div class="relative rounded-xl overflow-hidden border border-white/10 bg-black/60 max-w-lg w-full flex items-center justify-center min-h-[320px]">
        ...
        {:else}
          <div class="p-12 text-center space-y-4 max-w-sm">
            ...
            <div class="text-xs font-mono text-emerald-400 tracking-widest uppercase">STAY TUNED!</div>
      ...
      <div class="w-full text-center space-y-4 pt-2">
        ...
        <div class="pt-4 border-t border-white/[0.06] flex items-center justify-center">
          ...
          >
            <RefreshCw size={14} class="text-emerald-400" />
